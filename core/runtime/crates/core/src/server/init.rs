...
pub async fn init_app_state(args: InitArgs) -> Result<(bool, AppState), InitError> {
  ...
  let pg_pool = if let Some(pg_config) = config.postgres.as_ref() {
    ...
    let dbname = pg_config.database.as_deref().unwrap_or("postgres");
    ...
    let pg_config = tokio_postgres::Config::from_str(&config_str)
      .map_err(|e| InitError::Config(crate::config::ConfigError::Invalid(e.to_string())))?;

    let manager = bb8_postgres::PostgresConnectionManager::new(pg_config, NoTls);
    let pool = bb8::Pool::builder()
      .build(manager)
      .await
      .map_err(|e| InitError::CustomInit(e.to_string()))?;
