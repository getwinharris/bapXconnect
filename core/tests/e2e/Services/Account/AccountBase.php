...
trait AccountBase
{
    public function testCreateAccount(): array
    {
        ...
        $response = $this->client->call(Client::METHOD_POST, '/account', array_merge([
            ...
            'content-type' => 'application/json',
            'x-bapxdb-project' => $this->getProject()['$id'],
            'x-bapxdb-dev-key' => $this->getProject()['devKey'] ?? '',
        ]), [
            'userId' => ID::unique(),
            'email' => '',
            'password' => $password,
    ...
    public function testEmailOTPSession(): void
    {
        ...
        $this->assertEquals('otpuser@bapxdb.io', $lastEmail['to'][0]['address']);
        ...
        if ($isConsoleProject) {
            $this->assertStringContainsStringIgnoringCase('bapXdb logo', $lastEmail['html']);
        } else {
            $this->assertStringNotContainsStringIgnoringCase('bapXdb logo', $lastEmail['html']);
        }
        ...
        $this->assertArrayHasKey('targets', $response['body']);
        $this->assertEquals('otpuser@bapxdb.io', $response['body']['targets'][0]['identifier']);
    ...
    public function testFallbackForTrustedIp(): void
    {
        ...
        // call bapxdb directly to avoid proxy stripping the headers
