...
Function Runtimes allow you to execute code written in any language and form the basis of bapXdb's Cloud Functions! bapXdb's goal is to support as many function runtimes as possible.

## 1. Prerequisites

For a function runtime to work, two prerequisites **must** be met due to the way bapXdb's Runtime Execution Model works:
...
Note: Both Compiled and Interpreted languages work with bapXdb's execution model but are written in slightly different ways.

It's really easy to contribute to an open-source project, but when using GitHub, there are a few steps we need to follow. This section will take you step-by-step through the process of preparing your local version of bapXdb, where you can make any changes without affecting bapXdb right away.

> If you are experienced with GitHub or have made a pull request before, you can skip to [Implement new runtime](https://github.com/bapxdb/bapxdb/blob/master/docs/tutorials/add-runtime.md#2-implement-new-runtime).

### 1.1 Fork the bapXdb repository

Before making any changes, you will need to fork bapXdb's repository to keep branches on the official repo clean. To do that, visit [bapXdb's Runtime repository](https://github.com/bapxdb/runtimes) and click on the fork button.

[![Fork button](https://github.com/bapxdb/bapxdb/raw/master/docs/tutorials/images/fork.png)](https://github.com/bapxdb/bapxdb/blob/master/docs/tutorials/images/fork.png)

This will redirect you from `github.com/bapxdb/runtimes` to `github.com/YOUR_USERNAME/runtimes`, meaning all changes you do are only done inside your repository. Once you are there, click the highlighted `Code` button, copy the URL and clone the repository to your computer using the `git clone` command:
...
Within that folder you will need to create a few basic files that all bapXdb runtimes require:
...
Runtimes within bapXdb are created differently depending on whether they are compiled or interpreted. This is due to the fundamental differences between the two ways of running the code.
...
The `launch.sh` file for an interpreted runtime should extract the `/tmp/code.tar.gz` file that contains both the user's code and the dependencies. This tarball was created by bapXdb from the `/usr/code` folder and should install the dependencies that were pre-installed by the build stage and move them into the relevant locations for that runtime. It will then run the server ready for execution.
...
All errors that occur during the execution of a user's function **MUST** be returned using this JSON Object otherwise bapXdb will be unable to parse them for the user.
...
```bash
RUN groupadd -g 2000 bapxdb \
&& useradd -m -u 2001 -g bapxdb bapxdb
```
...
```
RUN ["chown", "-R", "bapxdb:bapxdb", "/usr/local/src"]
RUN ["chown", "-R", "bapxdb:bapxdb", "/usr/code"]
RUN ["chown", "-R", "bapxdb:bapxdb", "/usr/workspace"]
RUN ["chown", "-R", "bapxdb:bapxdb", "/usr/builtCode"]
```
...
$dart->addVersion('2.12', 'dart-runtime:2.12', 'bapxdb-ubuntu:20.04', [System::X86, System::ARM]);
$this->runtimes['dart'] = $dart;
```

This is an example of what you would do for a compiled language such as dart.

The first line is creating a new language entry. The first parameter is the internal name and the second one is the external one which is what the user will see in bapXdb.
...
For interpreted languages, this is normally the same as the Build Image, but for compiled languages, this can be either "bapxdb-alpine:3.13.6" or "bapxdb-ubuntu:20.04"
...
First of all, commit the changes with the message `Added XXX Runtime` and push it. This will publish a new branch to your forked version of bapXdb. If you visit it at `github.com/YOUR_USERNAME/runtimes`, you will see a new alert saying you are ready to submit a pull request. Follow the steps GitHub provides, and at the end, you will have your pull request submitted.
