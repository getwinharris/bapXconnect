...
# BAPX_UI_DIR should be the directory where env.py resides (bapx_ui/)
BAPX_UI_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR is the parent of BAPX_UI_DIR (backend/)
BACKEND_DIR = BAPX_UI_DIR.parent

# BASE_DIR is the parent of BACKEND_DIR (bapx-ui-dev/)
...
BAPX_UI_NAME = os.environ.get("BAPX_UI_NAME", "bapX AI")
BAPX_UI_FAVICON_URL = "/favicon.png"
...
if FROM_INIT_PY:
    PACKAGE_DATA = {"version": importlib.metadata.version("bapx-ui")}
else:
    try:
...
try:
    ...
    changelog_content = (pkgutil.get_data("bapx_ui", "CHANGELOG.md") or b"").decode()
...
BAPX_UI_BUILD_HASH = os.environ.get("BAPX_UI_BUILD_HASH", "dev-build")
...
if FROM_INIT_PY:
    NEW_DATA_DIR = Path(os.getenv("DATA_DIR", BAPX_UI_DIR / "data")).resolve()
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / "bapx_ui_data", "zip", DATA_DIR)

        # Remove the old data directory
        shutil.rmtree(DATA_DIR)

    DATA_DIR = Path(os.getenv("DATA_DIR", BAPX_UI_DIR / "data"))

STATIC_DIR = Path(os.getenv("STATIC_DIR", BAPX_UI_DIR / "static"))

FONTS_DIR = Path(os.getenv("FONTS_DIR", BAPX_UI_DIR / "static" / "fonts"))

FRONTEND_BUILD_DIR = Path(os.getenv("FRONTEND_BUILD_DIR", BASE_DIR / "build")).resolve()

if FROM_INIT_PY:
    FRONTEND_BUILD_DIR = Path(
        os.getenv("FRONTEND_BUILD_DIR", BAPX_UI_DIR / "frontend")
    ).resolve()
...
DATABASE_URL = os.environ.get("DATABASE_URL", f"sqlite:///{DATA_DIR}/bapx_ui.db")
...
if all(DB_VARS.values()):
    ...
    DATABASE_URL = f"sqlite+sqlcipher:///{DATA_DIR}/bapx_ui.db"
...
REDIS_KEY_PREFIX = os.environ.get("REDIS_KEY_PREFIX", "bapx-ui")
...
####################################
# BAPX_UI_AUTH (Required for security)
####################################

BAPX_UI_AUTH = os.environ.get("BAPX_UI_AUTH", "True").lower() == "true"
...
BAPX_UI_AUTH_TRUSTED_EMAIL_HEADER = os.environ.get(
    "BAPX_UI_AUTH_TRUSTED_EMAIL_HEADER", None
)
BAPX_UI_AUTH_TRUSTED_NAME_HEADER = os.environ.get("BAPX_UI_AUTH_TRUSTED_NAME_HEADER", None)
BAPX_UI_AUTH_TRUSTED_GROUPS_HEADER = os.environ.get(
    "BAPX_UI_AUTH_TRUSTED_GROUPS_HEADER", None
)
...
BAPX_UI_AUTH_SIGNOUT_REDIRECT_URL = os.environ.get(
    "BAPX_UI_AUTH_SIGNOUT_REDIRECT_URL", None
)
...
BAPX_UI_SECRET_KEY = os.environ.get(
    "BAPX_UI_SECRET_KEY",
    os.environ.get(
        "BAPX_UI_JWT_SECRET_KEY", "t0p-s3cr3t"
    ),  # DEPRECATED: remove at next major version
)

BAPX_UI_SESSION_COOKIE_SAME_SITE = os.environ.get("BAPX_UI_SESSION_COOKIE_SAME_SITE", "lax")

BAPX_UI_SESSION_COOKIE_SECURE = (
    os.environ.get("BAPX_UI_SESSION_COOKIE_SECURE", "false").lower() == "true"
)

BAPX_UI_AUTH_COOKIE_SAME_SITE = os.environ.get(
    "BAPX_UI_AUTH_COOKIE_SAME_SITE", BAPX_UI_SESSION_COOKIE_SAME_SITE
)

BAPX_UI_AUTH_COOKIE_SECURE = (
    os.environ.get(
        "BAPX_UI_AUTH_COOKIE_SECURE",
        os.environ.get("BAPX_UI_SESSION_COOKIE_SECURE", "false"),
    ).lower()
    == "true"
)

if BAPX_UI_AUTH and BAPX_UI_SECRET_KEY == "":
...
OAUTH_CLIENT_INFO_ENCRYPTION_KEY = os.environ.get(
    "OAUTH_CLIENT_INFO_ENCRYPTION_KEY", BAPX_UI_SECRET_KEY
)

OAUTH_SESSION_TOKEN_ENCRYPTION_KEY = os.environ.get(
    "OAUTH_SESSION_TOKEN_ENCRYPTION_KEY", BAPX_UI_SECRET_KEY
)
...
OTEL_SERVICE_NAME = os.environ.get("OTEL_SERVICE_NAME", "bapx-ui")
