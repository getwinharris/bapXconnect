...
We would :heart: you to contribute to bapXdb and help make it better! We want contributing to bapXdb to be fun, enjoyable, and educational for anyone and everyone. All contributions are welcome, including issues, and new docs, as well as updates and tweaks, blog posts, workshops, and more.

## Here for Hacktoberfest?
If you're here to contribute during Hacktoberfest, we're so happy to see you here. bapXdb has been a long-time participant of Hacktoberfest and we welcome you, whatever your experience level. This year, we're **only taking contributions for issues tagged** `hacktoberfest`, so we can focus our resources to support your contributions.

You can [find issues using this query](https://github.com/search?q=org%3Abapxdb+is%3Aopen+type%3Aissue+label%3Ahacktoberfest&type=issues).

## How to Start?

Welcome! We're excited that you're interested in contributing to bapXdb. To make sure your time is valued and your contributions are successful, please follow these steps before writing any code:

### 🔍 Step 1: Find an Issue

Browse open issues and look for ones labeled [good first issue](https://github.com/search?q=org%3Abapxdb+is%3Aopen+type%3Aissue+label%3A%22good+first+issue%22&type=issues) or [help wanted](https://github.com/search?q=org%3Abapxdb+is%3Aopen+type%3Aissue+label%3A%22help+wanted%22&type=issues).
...
Help us keep bapXdb open and inclusive. Please read and follow our [Code of Conduct](https://github.com/bapxdb/.github/blob/main/CODE_OF_CONDUCT.md).
...
4. Before you push your changes, make sure your code follows the `PSR12` coding standards, which is the standard that bapXdb currently follows. You can easily do this by running the formatter.
...
> If you just want to install bapXdb for day-to-day use and not as a contributor, you can reference the [installation guide](https://github.com/bapxdb/bapxdb#installation), the [getting started guide](https://bapxdb.io/docs/quick-starts), or the main [README](README.md) file.

```bash
git clone git@github.com:[YOUR_FORK_HERE]/bapxdb.git
...
To get proper autocompletion for all the different functions and classes in the codebase, you'll need to install bapXdb dependencies on your local machine. You can easily do that with PHP's package manager, [Composer](https://getcomposer.org/). If you don't have Composer installed, you can use the Docker Hub image to get the same result:
...
bapXdb's UI is built with [Svelte](https://svelte.dev/), [Svelte Kit](https://kit.svelte.dev/), and the [Pink Design](https://github.com/bapxdb/pink) component library. You can find the source code in the [bapXdb Console](https://github.com/bapxdb/console) repository.

To contribute to the UI, head to the [Contribution Guide](https://github.com/bapxdb/console/blob/main/CONTRIBUTING.md) of bapXdb Console.
...
We love to create issues that are good for beginners and label them as `good first issue` or `hacktoberfest`, but some more advanced topics might require extra knowledge. Below is a list of links you can use to learn about the more advanced topics that will help you master the bapXdb codebase.
...
- [MVVM](https://en.wikipedia.org/wiki/Model%E2%80%93view%E2%80%93viewmodel) - bapXdb console architecture
...
`bapxdb-worker-X` - Workers (`src/bapXdb/Platform/Workers/*`)
`bapxdb-task-X` - Tasks (`src/bapXdb/Platform/Tasks/*`)

Other containes should be named the same as their service, for example `redis` should just be called `redis`.

##### Security

- [bapXdb Auth and ACL](https://github.com/bapxdb/bapxdb/blob/master/docs/specs/authentication.drawio.svg)
...
```
src/bapXdb/Platform/Modules/[service]/Http/[resource]/[action].php
```
...
bapXdb's current structure is a combination of both [Monolithic](https://en.wikipedia.org/wiki/Monolithic_application) and [Microservice](https://en.wikipedia.org/wiki/Microservices) architectures.

---

## ![bapXdb](docs/specs/overview.drawio.svg)
...
bapXdb's main API container is designed as a monolithic app. This is a decision we made to allow us to develop the project faster while still being a very small team.

Although the bapXdb API is a monolithic app, it has a very clear separation of concern as each internal service or worker is separated by its container, which allows us to start breaking services for better maintenance and scalability as we grow.

### The Microservice Part

Each container in bapXdb is a microservice on its own. Each service is an independent process that can scale without regard to any of the other services.

Currently, all the bapXdb microservices are intended to communicate using the TCP protocol over a private network. With the exception of the public-facing port 80 and 443, which by default are used to expose the bapXdb HTTP API, you should **avoid exposing any other services' ports**.

## Ports

bapXdb dev version uses ports 80 and 443 as an entry point to the bapXdb API and console. We also expose multiple ports in the range of 9500-9504 for debugging some of the bapXdb containers on dev mode. If you have any conflicts with the ports running on your system, you can easily replace them by editing bapXdb's docker-compose.yml file and executing `docker compose up -d` command.

## Technology Stack

To start helping us to improve the bapXdb server by submitting code, prior knowledge of bapXdb's technology stack can help you get started.

bapXdb stack is a combination of a variety of open-source technologies and tools. bapXdb backend API is written primarily with PHP version 7 and above, on top of the [Utopia PHP framework](https://github.com/utopia-php/framework). The bapXdb frontend is built with tools like gulp, less, and [litespeed.js](https://github.com/litespeed-js). We use Docker as the container technology to package the bapXdb server for easy on-cloud, on-premise, or on-localhost integration.
...
bapXdb uses a package manager for managing code dependencies for both backend and frontend development. We try our best to avoid creating any unnecessary dependencies. New dependency to the project is subjected to a lead developer's review and approval.

Many of bapXdb's internal modules are also used as dependencies to allow other bapXdb projects to reuse them and as a way to contribute back to the community.

bapXdb uses [PHP's Composer](https://getcomposer.org/) for managing dependencies on the server-side and [JS NPM](https://www.npmjs.com/) for managing dependencies on the frontend side.

## Coding Standards

bapXdb follows the [PHP-FIG standards](https://www.php-fig.org/). Currently, we use both PSR-0 and PSR-12 for coding standards and autoloading standards.

We use prettier for our JS coding standards and auto-formatting for our code.

## Scalability, Speed, and Performance

bapXdb is built to scale. Please keep in mind that the bapXdb stack can run in different environments and different scales.

We intend bapXdb to be as easy to set up as possible in a single localhost, and to grow easily into a large environment with dozens and even hundreds of instances.
...
Security and privacy are extremely important to bapXdb, developers, and users alike. Make sure to follow the best industry standards and practices.

## Dependencies

Please avoid introducing new dependencies to bapXdb without consulting the team. New dependencies can be very helpful, but they also introduce new security and privacy risks, add complexity, and impact the total docker image size.

Adding a new dependency should have vital value for the product with minimum possible risk.

## Introducing New Features

We would :sparkling_heart: you to contribute to bapXdb, but we also want to ensure bapXdb is loyal to its vision and mission statement :pray:.

For us to find the right balance, please open an issue explaining your ideas before introducing a new pull request.

This will allow the bapXdb community to sufficiently discuss the new feature value and how it fits within the product roadmap and vision.

This is also important for the bapXdb lead developers to be able to provide technical input and potentially a different emphasis regarding the feature design and architecture. Some bigger features might need to go through our [RFC process](https://github.com/bapxdb/rfc).
...
In addition, you will also need to add some logic to the `reduce()` method of the Usage worker located in `/src/bapXdb/Platform/Workers/Usage.php`, like so:
...
To build a new version of the bapXdb server, all you need to do is run the build.sh file like this:

```bash
bash ./build.sh X.X.X
```

Before running the command, make sure you have proper write permissions to the bapXdb docker hub team.

**Build for Multicore**

```bash
docker buildx build --platform linux/amd64,linux/arm64,linux/arm/v6,linux/arm/v7,linux/arm64/v8,linux/ppc64le,linux/s390x -t bapxdb/bapxdb:dev --push .
```
...
1. Update the console spec file located at `app/config/specs/swagger2-<version-number>.console.json` using bapXdb Tasks. Run the `php app/cli.php specs version=<version-number> mode=normal` command in a running `bapxdb/bapxdb` container.
...
5. Copy `iife/sdk.js` to `bapxdb.js`.
...
bapXdb uses [XDebug](https://github.com/xdebug/xdebug) debugger, which can be made available during build of bapXdb. You can connect to the debugger using VS Code's [PHP Debug](https://marketplace.visualstudio.com/items?itemName=felixfbecker.php-debug) extension.

If you are in PHP Storm you don't need any plugin. Below are the settings required for remote debugger connection:

1. Set **DEBUG** build arg in **bapxdb** service in **docker-compose.yml** file.
2. If needed edit the **dev/xdebug.ini** file to your needs.
3. Launch your bapXdb instance while your debugger is listening for connections.

## Profiling
bapXdb uses XDebug [Profiler](https://xdebug.org/docs/profiler) for generating **CacheGrind** files. The generated file would be located in each of the `bapxdb` containers inside the `/tmp/xdebug` folder.
...
To run all tests manually, use the bapXdb Docker CLI from your terminal:

```bash
docker compose exec bapxdb test
```

To run unit tests use:

```bash
docker compose exec bapxdb test /usr/src/code/tests/unit
```

To run end-2-end tests use:

```bash
docker compose exec bapxdb test /usr/src/code/tests/e2e
```

To run end-2-end tests for a specific service use:

```bash
docker compose exec bapxdb test /usr/src/code/tests/e2e/Services/[ServiceName]
```

To run one specific test:

```bash
docker compose exec bapxdb vendor/bin/phpunit --filter [FunctionName]
```

## Benchmarking

You can use WRK Docker image to benchmark the server performance. Benchmarking is extremely useful when you want to compare how the server behaves before and after a change has been applied. Replace [APPWRITE_HOSTNAME_OR_IP] with your bapXdb server hostname or IP. Note that localhost is not accessible from inside the WRK container.
...
bapXdb Functions are automatically given a domain you can visit to execute the function. This domain has format `[SOMETHING].functions.localhost` unless you changed `_APP_DOMAIN_FUNCTIONS` environment variable. This default value works great when running bapXdb locally, but it can be impossible to use preview domains with Cloud woekspaces such as Gitpod or GitHub Codespaces.
...
```
https://8080-bapxdb-bapxdb-mjeb3ebilwv.ws-eu116.gitpod.io/ping?preview=672b3c7eab1ac523ccf5.functions.localhost
```
...
From time to time, our team will add tutorials that will help contributors find their way in the bapXdb source code. Below is a list of currently available tutorials:

- [Adding Support for a New OAuth2 Provider](./docs/tutorials/add-oauth2-provider.md)
- [bapXdb Environment Variables](./docs/tutorials/add-environment-variable.md)
- [Running in Production](https://bapxdb.io/docs/advanced/self-hosting/production)
- [Adding Storage Adapter](./docs/tutorials/add-storage-adapter.md)

## Other Ways to Help

Pull requests are great, but there are many other ways you can help bapXdb.

### Blogging & Speaking

Blogging, speaking about, or creating tutorials about one of bapXdb’s many features are great ways to get the word out about bapXdb. Mention [@bapxdb on Twitter](https://twitter.com/bapxdb) and/or [email team@bapxdb.io](mailto:team@bapxdb.io) so we can give pointers and tips and help you spread the word by promoting your content on the different bapXdb communication channels. Please add your blog posts and videos of talks to our [Awesome bapXdb](https://github.com/bapxdb/awesome-bapxdb) repo on GitHub.

### Presenting at Meetups

We encourage our contributors to present at meetups and conferences about your bapXdb projects. Your unique challenges and successes in building things with bapXdb can provide great speaking material. We’d love to review your talk abstract/CFP, so get in touch with us if you’d like some help!

### Sending Feedbacks and Reporting Bugs

Sending feedback is a great way for us to understand your different use cases of bapXdb better. If you had any issues, bugs, or want to share your experience, feel free to do so on our GitHub issues page or at our [Discord channel](https://discord.gg/GSeTUeA).

### Submitting New Ideas

If you think bapXdb could use a new feature, please open an issue on our GitHub repository, stating as much information as you have about your new idea and its implications. We would also use this issue to gather more information, get more feedback from the community, and have a proper discussion about the new feature.
...
Consider searching for bapXdb on Discord, GitHub, or StackOverflow to help someone who needs help. You can also help by teaching others how to contribute to bapXdb's repo!
