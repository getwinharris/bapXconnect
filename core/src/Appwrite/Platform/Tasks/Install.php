...
use bapXdb\Docker\Compose;
use bapXdb\Docker\Env;
...
{
    protected string $path = '/usr/src/code/bapxdb';
    ...
    public function __construct()
    {
        $this
            ...
            ->param('organization', 'bapxdb', new Text(0), 'Docker Registry organization', true)
            ->param('image', 'bapxdb', new Text(0), 'Main bapxdb docker image', true)
    ...
    public function action(string $httpPort, string $httpsPort, string $organization, string $image, string $interactive, bool $noStart): void
    {
        ...
        Console::success('Starting bapXdb installation...');
        ...
        if ($data !== false) {
            ...
            $bapxdb = $compose->getService('bapxdb');
            $oldVersion = $bapxdb?->getImageVersion();
            try {
        ...
        if ($exit !== 0) {
            $message = 'Failed to install bapXdb dockers';
            Console::error($message);
            Console::error($stderr);
            Console::exit($exit);
        } else {
            $message = 'bapXdb installed successfully';
