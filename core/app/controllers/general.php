...
use bapXdb\Auth\Key;
use bapXdb\Event\Certificate;
use bapXdb\Event\Event;
use bapXdb\Event\Func;
use bapXdb\Event\StatsUsage;
use bapXdb\Extend\Exception as bapXdbException;
use bapXdb\Network\Cors;
use bapXdb\Platform\bapXdb;
use bapXdb\SDK\Method;
use bapXdb\SDK\Response as SDKResponse;
use bapXdb\Transformation\Adapter\Preview;
use bapXdb\Transformation\Transformation;
use bapXdb\Utopia\Database\Documents\User as DBUser;
use bapXdb\Utopia\Request;
use bapXdb\Utopia\Request\Filters\V16 as RequestV16;
use bapXdb\Utopia\Request\Filters\V17 as RequestV17;
use bapXdb\Utopia\Request\Filters\V18 as RequestV18;
use bapXdb\Utopia\Request\Filters\V19 as RequestV19;
use bapXdb\Utopia\Request\Filters\V20 as RequestV20;
use bapXdb\Utopia\Request\Filters\V21 as RequestV21;
use bapXdb\Utopia\Response;
use bapXdb\Utopia\Response\Filters\V16 as ResponseV16;
use bapXdb\Utopia\Response\Filters\V17 as ResponseV17;
use bapXdb\Utopia\Response\Filters\V18 as ResponseV18;
use bapXdb\Utopia\Response\Filters\V19 as ResponseV19;
use bapXdb\Utopia\View;
...
function router(App $utopia, Database $dbForPlatform, callable $getProjectDB, SwooleRequest $swooleRequest, Request $request, Response $response, Log $log, Event $queueForEvents, StatsUsage $queueForStatsUsage, Func $queueForFunctions, Executor $executor, Reader $geodb, callable $isResourceBlocked, array $platform, string $previewHostname, Authorization $authorization, ?Key $apiKey)
{
    ...
    if ($rule->isEmpty()) {
        ...
        if ($host === $appDomainFunctions || $host === $appDomainSites) {
            throw new bapXdbException(bapXdbException::GENERAL_ACCESS_FORBIDDEN, 'This domain cannot be used for security reasons. Please use any subdomain instead.', view: $errorView);
        }

        if (\str_ends_with($host, $appDomainFunctions) || \str_ends_with($host, $appDomainSites)) {
            $exception = new bapXdbException(bapXdbException::RULE_NOT_FOUND, 'This domain is not connected to any bapXdb resources. Visit domains tab under function/site settings to configure it.', view: $errorView);
        ...
        if (!in_array($host, $platformHostnames)) {
            throw new bapXdbException(bapXdbException::GENERAL_ACCESS_FORBIDDEN, 'Router protection does not allow accessing bapXdb over this domain. Please add it as custom domain to your project or disable _APP_OPTIONS_ROUTER_PROTECTION environment variable.', view: $errorView);
        }
    ...
    if (!$project->isEmpty() && $project->getId() !== 'console') {
        ...
        /**
         * Set projectId to update the Error hook logger, since x-bapxdb-project is not available when executing custom domain function
         */
    ...
    if (array_key_exists('proxy', $project->getAttribute('services', []))) {
        ...
        if (!$status) {
            throw new bapXdbException(bapXdbException::GENERAL_SERVICE_DISABLED, view: $errorView);
        }
    }

    // Skip bapXdb Router for ACME challenge. Nessessary for certificate generation
    ...
    if ($type === 'deployment') {
        if (System::getEnv('_APP_OPTIONS_ROUTER_FORCE_HTTPS', 'disabled') === 'enabled') { // Force HTTPS
            if ($request->getProtocol() !== 'https') {
                if ($request->getMethod() !== Request::METHOD_GET) {
                    throw new bapXdbException(bapXdbException::GENERAL_PROTOCOL_UNSUPPORTED, 'Method unsupported over HTTP. Please use HTTPS instead.', view: $errorView);
                }
        ...
        if ($deployment->isEmpty()) {
            ...
            $exception = new bapXdbException(bapXdbException::DEPLOYMENT_NOT_FOUND, view: $errorView);
        ...
        */
        ...
        if ($resource->isEmpty() || !$resource->getAttribute('enabled')) {
            if ($type === 'functions') {
                throw new bapXdbException(bapXdbException::FUNCTION_NOT_FOUND, view: $errorView);
            } else {
                throw new bapXdbException(bapXdbException::SITE_NOT_FOUND, view: $errorView);
            }
        }

        if ($isResourceBlocked($project, $type === 'function' ? RESOURCE_TYPE_FUNCTIONS : RESOURCE_TYPE_SITES, $resource->getId())) {
            throw new bapXdbException(bapXdbException::GENERAL_RESOURCE_BLOCKED, view: $errorView);
        }
        ...
        if (\is_null($runtime)) {
            throw new bapXdbException(bapXdbException::FUNCTION_RUNTIME_UNSUPPORTED, 'Runtime "' . $resource->getAttribute('runtime', '') . '" is not supported', view: $errorView);
        }
        ...
        if (!$allowAnyStatus && $deployment->getAttribute('status') !== 'ready') {
            ...
            switch ($status) {
                case 'failed':
                    $exception = new bapXdbException(bapXdbException::BUILD_FAILED, view: $errorView);
                    ...
                    break;
                case 'canceled':
                    $exception = new bapXdbException(bapXdbException::BUILD_CANCELED, view: $errorView);
                    ...
                    break;
                default:
                    $exception = new bapXdbException(bapXdbException::BUILD_NOT_READY, view: $errorView);
                    $ctaUrl = '/console/project-' . $region . '-' . $project->getId() . '/sites/site-' . $resource->getId() . '/deployments/deployment-' . $deployment->getId();
        ...
        if ($type === 'function') {
            ...
            if (!(\in_array('any', $permissions)) && !(\in_array('guests', $permissions))) {
                $exception = new bapXdbException(bapXdbException::FUNCTION_EXECUTE_PERMISSION_MISSING, view: $errorView);
        ...
        $headers['x-bapxdb-execution-id'] = $executionId ?? '';
        $headers['x-bapxdb-user-id'] = '';
        $headers['x-bapxdb-country-code'] = '';
        $headers['x-bapxdb-continent-code'] = '';
        $headers['x-bapxdb-continent-eu'] = 'false';
        $ip = $request->getIP();
        $headers['x-bapxdb-client-ip'] = $ip;
        ...
        $headers['x-bapxdb-key'] = API_KEY_DYNAMIC . '_' . $jwtKey;
        $headers['x-bapxdb-trigger'] = 'http';
        $headers['x-bapxdb-user-jwt'] = '';
        ...
        if (!empty($ip)) {
            ...
            if ($record) {
                ...
                $headers['x-bapxdb-country-code'] = $record['country']['iso_code'] ?? '';
                $headers['x-bapxdb-continent-code'] = $record['continent']['code'] ?? '';
                $headers['x-bapxdb-continent-eu'] = (\in_array($record['country']['iso_code'], $eu)) ? 'true' : 'false';
            }
        }
        ...
        if ($version === 'v2') {
            $vars = \array_merge($vars, [
                'APPWRITE_FUNCTION_TRIGGER' => $headers['x-bapxdb-trigger'] ?? '',
                'APPWRITE_FUNCTION_DATA' => $body ?? '',
                'APPWRITE_FUNCTION_USER_ID' => $headers['x-bapxdb-user-id'] ?? '',
                'APPWRITE_FUNCTION_JWT' => $headers['x-bapxdb-user-jwt'] ?? ''
            ]);
        }
        ...
        try {
            ...
            if ($deployment->getAttribute('resourceType') === 'functions') {
                $headerOverrides['x-bapxdb-execution-id'] = $execution->getId();
            } elseif ($deployment->getAttribute('resourceType') === 'sites') {
                $headerOverrides['x-bapxdb-log-id'] = $execution->getId();
            }
            ...
            if ($th instanceof bapXdbException) {
                throw $th;
            }
        } finally {
            if ($type === 'function') {
        ...
        throw new bapXdbException(bapXdbException::GENERAL_SERVER_ERROR, 'Unknown resource type ' . $type, view: $errorView);
    }
...
App::init()
    ...
    ->action(function (Document $project, string $mode) {
        if ($mode === APP_MODE_ADMIN && $project->getId() === 'console') {
            throw new bapXdbException(bapXdbException::GENERAL_BAD_REQUEST, 'Admin mode is not allowed for console project');
        }
    });
...
App::init()
    ...
    ->action(function (Document $project, Request $request) {
        if ($project->getId() === 'console') {
            $message = empty($request->getHeader('x-bapxdb-project')) ?
                'No bapXdb project was specified. Please specify your project ID when initializing your bapXdb SDK.' :
                'This endpoint is not available for the console project. The bapXdb Console is a reserved project ID and cannot be used with the bapXdb SDKs and APIs. Please check if your project ID is correct.';
            throw new bapXdbException(bapXdbException::GENERAL_ACCESS_FORBIDDEN, $message);
        }
    });
...
App::init()
    ...
    ->action(function (App $utopia, SwooleRequest $swooleRequest, Request $request, Response $response, Log $log, Document $project, Database $dbForPlatform, callable $getProjectDB, Locale $locale, array $localeCodes, Reader $geodb, StatsUsage $queueForStatsUsage, Event $queueForEvents, Func $queueForFunctions, Executor $executor, array $platform, callable $isResourceBlocked, string $previewHostname, Document $devKey, ?Key $apiKey, Cors $cors, Authorization $authorization) {
        ...
        */
        ...
        $requestFormat = $request->getHeader('x-bapxdb-response-format', System::getEnv('_APP_SYSTEM_RESPONSE_FORMAT', ''));
        if ($requestFormat) {
        ...
        $localeParam = (string) $request->getParam('locale', $request->getHeader('x-bapxdb-locale', ''));
         ...
         * Response format
         */
        $responseFormat = $request->getHeader('x-bapxdb-response-format', System::getEnv('_APP_SYSTEM_RESPONSE_FORMAT', ''));
        if ($responseFormat) {
            ...
            if (version_compare($responseFormat, APP_VERSION_STABLE, '>')) {
                $warnings[] = "The current SDK is built for bapXdb " . $responseFormat . ". However, the current bapXdb server version is " . APP_VERSION_STABLE . ". Please downgrade your SDK to match the bapXdb version: https://bapxdb.io/docs/sdks";
            }
        }

        // Add bapXdb warning headers
        if (!empty($warnings)) {
            $response->addHeader('X-bapXdb-Warning', implode(';', $warnings));
        }
        ...
        if (System::getEnv('_APP_OPTIONS_FORCE_HTTPS', 'disabled') === 'enabled') { // Force HTTPS
            if ($request->getProtocol() !== 'https' && ($swooleRequest->header['host'] ?? '') !== 'localhost') { // localhost allowed for proxy, APP_HOSTNAME_INTERNAL allowed for migrations
                if ($request->getMethod() !== Request::METHOD_GET) {
                    throw new bapXdbException(bapXdbException::GENERAL_PROTOCOL_UNSUPPORTED, 'Method unsupported over HTTP. Please use HTTPS instead.');
                }
...
App::init()
    ...
    ->action(function (Request $request, Response $response, Cors $cors, Document $devKey, Validator $originValidator) {
        ...
        if (empty($origin) || !$devKey->isEmpty() || !empty($request->getHeader('x-bapxdb-key'))) {
            return;
        }
        ...
        if (!$originValidator->isValid($origin)) {
            throw new bapXdbException(bapXdbException::GENERAL_UNKNOWN_ORIGIN, $originValidator->getDescription());
        }
    });
    ...
    ->action(function (Throwable $error, App $utopia, Request $request, Response $response, Document $project, ?Logger $logger, Log $log, StatsUsage $queueForStatsUsage, Document $devKey, Authorization $authorization) {
        ...
        switch ($class) {
            case 'Utopia\Exception':
                $error = new bapXdbException(bapXdbException::GENERAL_UNKNOWN, $message, $code, $error);
                switch ($code) {
                    case 400:
                        $error->setType(bapXdbException::GENERAL_ARGUMENT_INVALID);
                        break;
                    case 404:
                        $error->setType(bapXdbException::GENERAL_ROUTE_NOT_FOUND);
                        break;
                }
                break;
            case 'Utopia\Database\Exception\Authorization':
                $error = new bapXdbException(bapXdbException::USER_UNAUTHORIZED);
                break;
            case 'Utopia\Database\Exception\Timeout':
                $error = new bapXdbException(bapXdbException::DATABASE_TIMEOUT, previous: $error);
                break;
        }
        ...
        if ($error instanceof bapXdbException) {
        ...
        if ($logger && $publish) {
            ...
            $log->addTag('locale', (string)$request->getParam('locale', $request->getHeader('x-bapxdb-locale', '')));
        ...
        /** Wrap all exceptions inside bapXdb\Extend\Exception */
        if (!($error instanceof bapXdbException)) {
            $error = new bapXdbException(bapXdbException::GENERAL_UNKNOWN, $message, $code, $error);
        }
...
App::get('/.well-known/acme-challenge/*')
    ...
    ->action(function (Request $request, Response $response) {
        ...
        if (!$validator->isValid($token) || \count($uriChunks) !== 4) {
            throw new bapXdbException(bapXdbException::GENERAL_ARGUMENT_INVALID, 'Invalid challenge token.');
        }
        ...
        if (!$base) {
            throw new bapXdbException(bapXdbException::GENERAL_SERVER_ERROR, 'Storage error');
        }

        if (!$absolute) {
            throw new bapXdbException(bapXdbException::GENERAL_ROUTE_NOT_FOUND, 'Unknown path');
        }

        if (!\substr($absolute, 0, \strlen($base)) === $base) {
            throw new bapXdbException(bapXdbException::GENERAL_UNAUTHORIZED_SCOPE, 'Invalid path');
        }
        ...
        if (!$content) {
            throw new bapXdbException(bapXdbException::GENERAL_SERVER_ERROR, 'Failed to get contents');
        }
...
App::get('/v1/ping')
    ->groups(['api', 'general'])
    ->desc('Test the connection between the bapXdb and the SDK.')
    ->label('scope', 'global')
    ...
    ->action(function (Response $response, Document $project, Database $dbForPlatform, Event $queueForEvents, Authorization $authorization) {
        if ($project->isEmpty() || $project->getId() === 'console') {
            throw new bapXdbException(bapXdbException::PROJECT_NOT_FOUND);
        }
...
App::wildcard()
    ...
    ->label('scope', 'global')
    ->action(function () {
        throw new bapXdbException(bapXdbException::GENERAL_ROUTE_NOT_FOUND);
    });
