...
use bapXdb\Utopia\Database\Validator\Queries\Migrations;
...
use Utopia\Migration\Sources\bapXdb;
...
App::post('/v1/migrations/bapxdb')
    ->groups(['api', 'migrations'])
    ->desc('Create bapXdb migration')
    ...
    ->label('sdk', new Method(
        ...
        group: null,
        name: 'createbapXdbMigration',
        description: '/docs/references/migrations/migration-bapxdb.md',
        auth: [AuthType::ADMIN],
        responses: [
            new SDKResponse(
    ...
    ->param('resources', [], new ArrayList(new WhiteList(bapXdb::getSupportedResources())), 'List of resources to migrate')
    ->param('endpoint', '', new URL(), 'Source bapXdb endpoint')
    ...
    ->action(function (array $resources, string $endpoint, string $projectId, string $apiKey, Response $response, Database $dbForProject, Document $project, array $platform, Document $user, Event $queueForEvents, Migration $queueForMigrations) {
        $migration = $dbForProject->createDocument('migrations', new Document([
            ...
            'status' => 'pending',
            'stage' => 'init',
            'source' => bapXdb::getName(),
            'destination' => bapXdb::getName(),
            'credentials' => [
                'endpoint' => $endpoint,
                'projectId' => $projectId,
                'apiKey' => $apiKey,
...
App::post('/v1/migrations/csv/exports')
    ...
    ->param('queries', [], new ArrayList(new Text(0)), 'Array of query strings generated using the Query class provided by the SDK to filter documents to export. [Learn more about queries](https://bapxdb.io/docs/databases#querying-documents). Maximum of ' . APP_LIMIT_ARRAY_PARAMS_SIZE . ' queries are allowed, each ' . APP_LIMIT_ARRAY_ELEMENT_SIZE . ' characters long.', true)
...
App::get('/v1/migrations')
    ...
    ->param('queries', [], new Migrations(), 'Array of query strings generated using the Query class provided by the SDK. [Learn more about queries](https://bapxdb.io/docs/databases#querying-documents). Maximum of ' . APP_LIMIT_ARRAY_PARAMS_SIZE . ' queries are allowed, each ' . APP_LIMIT_ARRAY_ELEMENT_SIZE . ' characters long. You may filter on the following attributes: ' . implode(', ', Migrations::ALLOWED_ATTRIBUTES), true)
...
App::get('/v1/migrations/bapxdb/report')
    ->groups(['api', 'migrations'])
    ->desc('Get bapXdb migration report')
    ->label('scope', 'migrations.write')
    ->label('sdk', new Method(
        namespace: 'migrations',
        group: null,
        name: 'getbapXdbReport',
        description: '/docs/references/migrations/migration-bapxdb-report.md',
        auth: [AuthType::ADMIN],
        responses: [
            new SDKResponse(
                code: Response::STATUS_CODE_OK,
    ...
    ->param('resources', [], new ArrayList(new WhiteList(bapXdb::getSupportedResources())), 'List of resources to migrate')
    ->param('endpoint', '', new URL(), "Source's bapXdb Endpoint")
    ...
    ->action(function (array $resources, string $endpoint, string $projectID, string $key, Response $response) {
        ...
        $bapxdb = new bapXdb($projectID, $endpoint, $key);

        try {
            $report = $bapxdb->report($resources);
        } catch (\Throwable $e) {
            switch ($e->getCode()) {
                case 401:
                    throw new Exception(Exception::GENERAL_UNAUTHORIZED_SCOPE, 'Source Error: ' . $e->getMessage());
                case 429:
                    throw new Exception(Exception::GENERAL_RATE_LIMIT_EXCEEDED, 'Source Error: Rate Limit Exceeded, Is your Cloud Provider blocking bapXdb\'s IP?');
                case 500:
