...
class HTTPTest extends Scope
{
    ...
    {
        parent::setUp();
        $this->client->setEndpoint('http://bapxdb.test');
    }
    ...
    public function testOptions()
    {
        ...
        $this->assertEquals('bapXdb', $response['headers']['server']);
        $this->assertEquals('GET, POST, PUT, PATCH, DELETE', $response['headers']['access-control-allow-methods']);
        $this->assertEquals('Accept, Origin, Cookie, Set-Cookie, Content-Type, Content-Range, X-bapXdb-Project, X-bapXdb-Key, X-bapXdb-Dev-Key, X-bapXdb-Locale, X-bapXdb-Mode, X-bapXdb-JWT, X-bapXdb-Response-Format, X-bapXdb-Timeout, X-bapXdb-ID, X-bapXdb-Timestamp, X-bapXdb-Session, X-SDK-Version, X-SDK-Name, X-SDK-Language, X-SDK-Platform, X-SDK-GraphQL, X-SDK-Profile, Range, Cache-Control, Expires, Pragma, X-Fallback-Cookies, X-Requested-With, X-Forwarded-For, X-Forwarded-User-Agent', $response['headers']['access-control-allow-headers']);
        $this->assertEquals('X-bapXdb-Session, X-Fallback-Cookies', $response['headers']['access-control-expose-headers']);
    ...
    public function testPreflight()
    {
        ...
        $response = $this->client->call(Client::METHOD_OPTIONS, $endpoint, [
            ...
            'access-control-request-headers' => 'X-bapXdb-Project',
