...
use bapXdb\Utopia\Database\Validator\Queries\Installations;
...
App::get('/v1/vcs/github/callback')
    ...
    ->action(function (string $providerInstallationId, string $setupAction, string $state, string $code, GitHub $github, Document $user, Document $project, Response $response, Database $dbForPlatform, array $platform) {
        if (empty($state)) {
            $error = 'Installation requests from organisation members for the bapXdb GitHub App are currently unsupported. To proceed with the installation, login to the bapXdb Console and install the GitHub App.';
        ...
        if (!empty($providerInstallationId)) {
            ...
            $error = 'Installation of the bapXdb GitHub App on organization accounts is restricted to organization owners. As a member of the organization, you do not have the necessary permissions to install this GitHub App. Please contact the organization owner to create the installation from the bapXdb console.';
...
App::get('/v1/vcs/installations')
    ...
    ->param('queries', [], new Installations(), 'Array of query strings generated using the Query class provided by the SDK. [Learn more about queries](https://bapxdb.io/docs/queries). Maximum of ' . APP_LIMIT_ARRAY_PARAMS_SIZE . ' queries are allowed, each ' . APP_LIMIT_ARRAY_ELEMENT_SIZE . ' characters long. You may filter on the following attributes: ' . implode(', ', Installations::ALLOWED_ATTRIBUTES), true)
    ->param('search', '', new Text(256), 'Search term to filter your list results. Max length: 256 chars.', true)
