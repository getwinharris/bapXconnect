...
class AccountCustomClientTest extends Scope
{
    ...
    {
        $email = uniqid() . 'session-alert@bapxdb.io';
        ...
        $this->assertEquals('otpuser2@bapxdb.io', $lastEmail['to'][0]['address']);
    ...
    public function testCreatePhone(): array
    {
        ...
        $smsRequest = $this->assertLastRequest(function (array $request) use ($number) {
            $this->assertEquals('bapXdb Mock Message Sender', $request['headers']['User-Agent']);
