...
class FunctionsCustomServerTest extends Scope
{
    ...
    public function testCreateExecution($data): array
    {
        ...
        foreach ($execution['body']['responseHeaders'] as $header) {
            if ($header['name'] === 'x-bapxdb-execution-id') {
    ...
    public function testEventTrigger()
    {
        ...
        $this->assertEventually(function () use ($functionId, $userId) {
            ...
            $this->assertEmpty($headers['x-bapxdb-client-ip'] ?? '');
        }, 20_000, 500);
    ...
    public function testFunctionsDomain()
    {
        ...
        $this->assertArrayHasKey('x-bapxdb-execution-id', $response['headers']);
        $this->assertNotEmpty($response['headers']['x-bapxdb-execution-id']);
    ...
    public function testResponseFilters()
    {
        ...
        $response = $this->client->call(Client::METHOD_POST, '/functions', array_merge([
            'content-type' => 'application/json',
            'x-bapxdb-project' => $this->getProject()['$id'],
            'x-bapxdb-response-format' => '1.5.0', // add response format header
        ], $this->getHeaders()), [
            'functionId' => ID::unique(),
            'name' => 'Test',
            'runtime' => 'node-22',
    ...
    public function testRequestFilters()
    {
        ...
        $response = $this->client->call(
            ...
            array_merge([
                'content-type' => 'application/json',
                'x-bapxdb-project' => $this->getProject()['$id'],
                'x-bapxdb-response-format' => '1.4.0', // Set response format for 1.4 syntax
            ], $this->getHeaders()),
            [
