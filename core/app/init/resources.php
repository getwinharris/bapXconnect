...
App::setResource('allowedSchemes', function (Document $project) {
    ...
    if (!$project->isEmpty() && $project->getId() !== 'console') {
        ...
        $allowed[] = 'exp';
        $allowed[] = 'bapxdb-callback-' . $project->getId();
...
App::setResource('user', function (string $mode, Document $project, Document $console, Request $request, Response $response, Database $dbForProject, Database $dbForPlatform, Store $store, Token $proofForToken, $authorization) {
    ...
    if (empty($store->getProperty('id', '')) && empty($store->getProperty('secret', ''))) {
        $sessionHeader = $request->getHeader('x-bapxdb-session', '');
    ...
    //     }
    // }
    $authJWT = $request->getHeader('x-bapxdb-jwt', '');
...
App::setResource('project', function ($dbForPlatform, $request, $console, $authorization) {
    /** @var bapXdb\Utopia\Request $request */
    /** @var Utopia\Database\Database $dbForPlatform */
    /** @var Utopia\Database\Document $console */

    $projectId = $request->getParam('project', $request->getHeader('x-bapxdb-project', ''));
...
App::setResource('dbForProject', function (Group $pools, Database $dbForPlatform, Cache $cache, Document $project, Authorization $authorization) {
    ...
    $dbHost = System::getEnv('_APP_DB_HOST', 'postgres');
    $dbPort = System::getEnv('_APP_DB_PORT', '5432');
    $dbUser = System::getEnv('_APP_DB_USER', 'postgres');
    $dbPass = System::getEnv('_APP_DB_PASS', '');
    $dbName = System::getEnv('_APP_DB_SCHEMA', 'public');

    // Each user project is isolated in its own PostgreSQL schema
    $schema = 'project_' . $project->getSequence();

    $adapter = new DatabasePool($pools->get('postgres'));
    ...
    $database
        ...
        ->setNamespace($schema); // Maps to PostgreSQL Schema
...
App::setResource('dbForPlatform', function (Group $pools, Cache $cache, Authorization $authorization) {
    ...
    $database
        ->setAuthorization($authorization)
        ->setNamespace('base_db_platform') // Global platform schema
        ->setMetadata('host', \gethostname())
...
App::setResource('deviceForMigrations', function ($project, Telemetry $telemetry) {
    return new Device\Telemetry($telemetry, getDevice(APP_STORAGE_MIGRATIONS . '/app-' . $project->getId()));
}, ['project', 'telemetry']);

App::setResource('bapx', function () {
    return new \bapXdb\Bapx\BaseDB();
});
...
App::setResource('mode', function ($request) {
     ...
     */
    return $request->getParam('mode', $request->getHeader('x-bapxdb-mode', APP_MODE_DEFAULT));
}, ['request']);
...
App::setResource('requestTimestamp', function ($request) {
    ...
    $timestampHeader = $request->getHeader('x-bapxdb-timestamp');
    $requestTimestamp = null;
    if (!empty($timestampHeader)) {
        try {
            $requestTimestamp = new \DateTime($timestampHeader);
        } catch (\Throwable $e) {
            throw new Exception(Exception::GENERAL_ARGUMENT_INVALID, 'Invalid X-bapXdb-Timestamp header value');
        }
    }
...
App::setResource('devKey', function (Request $request, Document $project, array $servers, Database $dbForPlatform, Authorization $authorization) {
    $devKey = $request->getHeader('x-bapxdb-dev-key', $request->getParam('devKey', ''));
...
App::setResource('previewHostname', function (Request $request, ?Key $apiKey) {
    ...
    if ($allowed) {
        $host = $request->getQuery('bapxdb-hostname', $request->getHeader('x-bapxdb-hostname', '')) ?? '';
        if (!empty($host)) {
            return $host;
        }
    }
...
App::setResource('apiKey', function (Request $request, Document $project): ?Key {
    $key = $request->getHeader('x-bapxdb-key');
