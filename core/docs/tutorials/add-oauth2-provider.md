...
OAuth2 providers help users to log in to the apps and websites without the need to provide passwords or any other type of credentials. bapXdb's goal is to have support from as many **major** OAuth2 providers as possible.
...
It's really easy to contribute to an open source project, but when using GitHub, there are a few steps we need to follow. This section will take you step-by-step through the process of preparing your own local version of bapXdb, where you can make any changes without affecting bapXdb right away.
...
```bash
src/bapXdb/Auth/OAuth2/XXX.php
```
...
namespace bapXdb\Auth\OAuth2;

use bapXdb\Auth\OAuth2;
...
After you finish adding your new provider to bapXdb, you should be able to see it in your bapXdb console. Navigate to 'Project > Users > Providers' and check your new provider's settings form.

> To start the bapXdb console from the source code, you can simply run `docker compose up -d'.

Add credentials and check both a successful and a failed login (where the user denies integration on the provider page).

You can test your OAuth2 provider by trying to login using the [OAuth2 method](https://bapxdb.io/docs/references/cloud/client-web/account#createOAuth2Session) when integrating the bapXdb Web SDK in a demo app.
...
First of all, commit the changes with the message `Added XXX OAuth2 Provider` and push it. This will publish a new branch to your forked version of bapXdb. If you visit it at `github.com/YOUR_USERNAME/bapxdb`, you will see a new alert saying you are ready to submit a pull request. Follow the steps GitHub provides, and at the end, you will have your pull request submitted.
...
3. In your provider class `src/bapXdb/Auth/OAuth2/[Provider].php` add logic to decode the JSON using the same property names.
