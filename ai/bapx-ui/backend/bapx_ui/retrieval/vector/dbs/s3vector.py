from bapx_ui.retrieval.vector.utils import process_metadata
from bapx_ui.retrieval.vector.main import (
    VectorDBBase,
    VectorItem,
    GetResult,
    SearchResult,
)
from bapx_ui.config import S3_VECTOR_BUCKET_NAME, S3_VECTOR_REGION
...
class S3VectorClient(VectorDBBase):
    """
    AWS S3 Vector integration for bapX UI Knowledge.
    """
    ...
    def _filter_metadata(
        ...
        # Keep only the first 10 keys, prioritizing important ones based on actual bapX UI metadata
        important_keys = [
    ...
    def get(self, collection_name: str) -> Optional[GetResult]:
        ...
        try:
            ...
            # The bapX UI GetResult expects lists of lists, so we wrap each list
            if all_ids:
                return GetResult(
