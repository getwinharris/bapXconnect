...
from bapx_ui.internal.db import Base, get_db
...
def run_migrations():
    ...
    try:
        ...
        alembic_cfg = Config(BAPX_UI_DIR / "alembic.ini")

        # Set the script location dynamically
        migrations_path = BAPX_UI_DIR / "migrations"
...
if JWT_EXPIRES_IN.value == "-1":
    log.warning(
        ...
        "    See: https://docs.openbapx_ui.com/getting-started/env-configuration\n"
    )
...
STATIC_DIR = Path(os.getenv("STATIC_DIR", BAPX_UI_DIR / "static")).resolve()
...
if CUSTOM_NAME:
    try:
        r = requests.get(f"https://api.openbapx_ui.com/api/v1/custom/{CUSTOM_NAME}")
        data = r.json()
        if r.ok:
            if "logo" in data:
                BAPX_UI_FAVICON_URL = url = (
                    f"https://api.openbapx_ui.com{data['logo']}"
                    if data["logo"][0] == "/"
                    else data["logo"]
                )
            ...
            if "splash" in data:
                url = (
                    f"https://api.openbapx_ui.com{data['splash']}"
                    if data["splash"][0] == "/"
                    else data["splash"]
                )
...
if ENV == "prod":
    if OLLAMA_BASE_URL == "/ollama" and not K8S_FLAG:
        if USE_OLLAMA_DOCKER.lower() == "true":
            # if you use all-in-one docker container (bapX UI + Ollama)
            # with the docker build arg USE_OLLAMA=true (--build-arg="USE_OLLAMA=true") this only works with http://localhost:11434
            OLLAMA_BASE_URL = "http://localhost:11434"
        else:
            OLLAMA_BASE_URL = "http://host.docker.internal:11434"
    elif K8S_FLAG:
        OLLAMA_BASE_URL = "http://ollama-service.bapx-ui.svc.cluster.local:11434"
...
GLM_API_BASE_URL = os.environ.get("GLM_API_BASE_URL", "")
...
BAPX_UI_URL = PersistentConfig("BAPX_UI_URL", "bapx_ui.url", os.environ.get("BAPX_UI_URL", ""))
...
DEFAULT_ARENA_MODEL = {
    ...
    "name": "bapX",
    "meta": {
        "profile_image_url": "/static/logo.png",
        "description": "bapX - Sovereign Intelligence Hub",
        "model_ids": None,
    },
}
...
try:
    banners = json.loads(os.environ.get("BAPX_UI_BANNERS", "[]"))
    banners = [BannerModel(**banner) for banner in banners]
except Exception as e:
    log.exception(f"Error loading BAPX_UI_BANNERS: {e}")
    banners = []

BAPX_UI_BANNERS = PersistentConfig("BAPX_UI_BANNERS", "ui.banners", banners)
...
MILVUS_COLLECTION_PREFIX = os.environ.get("MILVUS_COLLECTION_PREFIX", "bapx_ui")
...
QDRANT_COLLECTION_PREFIX = os.environ.get("QDRANT_COLLECTION_PREFIX", "bapx-ui")
...
ELASTICSEARCH_INDEX_PREFIX = os.environ.get(
    "ELASTICSEARCH_INDEX_PREFIX", "bapx_ui_collections"
)
# Pgvector
...
PINECONE_INDEX_NAME = os.getenv("PINECONE_INDEX_NAME", "bapx-ui-index")
...
WEB_SEARCH_RESULT_COUNT = PersistentConfig(
    ...
    int(os.getenv("WEB_SEARCH_RESULT_COUNT", "50")),
)
