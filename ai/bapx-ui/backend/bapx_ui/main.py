...
from bapx_ui.utils import logger
from bapx_ui.utils.audit import AuditLevel, AuditLoggingMiddleware
from bapx_ui.utils.logger import start_logger
from bapx_ui.socket.main import (
    MODELS,
    app as socket_app,
...
from bapx_ui.routers.retrieval import (
...
from bapx_ui.internal.db import Session, engine

from bapx_ui.models.functions import Functions
from bapx_ui.models.models import Models
from bapx_ui.models.users import UserModel, Users
from bapx_ui.models.chats import Chats
...
from bapx_ui.env import (
    ...
    BAPX_UI_BUILD_HASH,
    BAPX_UI_SECRET_KEY,
    BAPX_UI_SESSION_COOKIE_SAME_SITE,
    BAPX_UI_SESSION_COOKIE_SECURE,
    ENABLE_SIGNUP_PASSWORD_CONFIRMATION,
    BAPX_UI_AUTH_TRUSTED_EMAIL_HEADER,
    BAPX_UI_AUTH_TRUSTED_NAME_HEADER,
    BAPX_UI_AUTH_SIGNOUT_REDIRECT_URL,
    # SCIM
    ENABLE_SCIM,
    SCIM_TOKEN,
...
from bapx_ui.utils.models import (
    get_all_models,
    get_all_base_models,
    check_model_access,
    get_filtered_models,
)
from bapx_ui.utils.chat import (
    generate_chat_completion as chat_completion_handler,
    chat_completed as chat_completed_handler,
    chat_action as chat_action_handler,
)
from bapx_ui.utils.embeddings import generate_embeddings
from bapx_ui.utils.middleware import process_chat_payload, process_chat_response
from bapx_ui.utils.access_control import has_access

from bapx_ui.utils.auth import (
    get_license_data,
...
from bapx_ui.utils.plugin import install_tool_and_function_dependencies
from bapx_ui.utils.oauth import (
...
from bapx_ui.utils.security_headers import SecurityHeadersMiddleware
from bapx_ui.utils.redis import get_redis_connection

from bapx_ui.tasks import (
...
from bapx_ui.utils.redis import get_sentinels_from_env
...
{f"Log: {BAPX_UI_BUILD_HASH}" if BAPX_UI_BUILD_HASH != "dev-build" else ""}
"""
)
...
app.state.BAPX_UI_NAME = BAPX_UI_NAME
...
if ENABLE_OTEL:
    from bapx_ui.utils.telemetry.setup import setup as setup_opentelemetry
...
app.state.config.BAPX_UI_URL = BAPX_UI_URL
...
app.state.config.BANNERS = BAPX_UI_BANNERS
...
app.state.AUTH_TRUSTED_EMAIL_HEADER = BAPX_UI_AUTH_TRUSTED_EMAIL_HEADER
app.state.AUTH_TRUSTED_NAME_HEADER = BAPX_UI_AUTH_TRUSTED_NAME_HEADER
app.state.BAPX_UI_AUTH_SIGNOUT_REDIRECT_URL = BAPX_UI_AUTH_SIGNOUT_REDIRECT_URL
        ...
        if request.method == "GET":
            ...
            if "shared" in query_params and len(query_params["shared"]) > 0:
                ...
                if text:
                    ...
                    if urls:
                        from bapx_ui.retrieval.loaders.youtube import _parse_video_id
...
@app.middleware("http")
async def save_session_after_request(request: Request, call_next):
    response = await call_next(request)
    # log.debug("Save session after request")
    Session.commit()
    return response
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for bapX UI.
...
try:
    if ENABLE_STAR_SESSIONS_MIDDLEWARE:
        ...
        app.add_middleware(
            ...
            cookie_same_site=BAPX_UI_SESSION_COOKIE_SAME_SITE,
            cookie_https_only=BAPX_UI_SESSION_COOKIE_SECURE,
        )
    ...
    app.add_middleware(
        SessionMiddleware,
        secret_key=BAPX_UI_SECRET_KEY,
        session_cookie="owui-session",
        same_site=BAPX_UI_SESSION_COOKIE_SAME_SITE,
        https_only=BAPX_UI_SESSION_COOKIE_SECURE,
    )
...
async def get_manifest_json():
    if app.state.EXTERNAL_PWA_MANIFEST_URL:
        ...
        return {
            "name": app.state.BAPX_UI_NAME,
            "short_name": app.state.BAPX_UI_NAME,
            "description": f"{app.state.BAPX_UI_NAME} is an open, extensible, user-friendly interface for AI that adapts to your workflow.",
            "start_url": "/",
            "display": "standalone",
...
async def get_opensearch_xml():
    ...
    <ShortName>{app.state.BAPX_UI_NAME}</ShortName>
    <Description>Search {app.state.BAPX_UI_NAME}</Description>
    <InputEncoding>UTF-8</InputEncoding>
    <Image width="16" height="16" type="image/x-icon">{app.state.config.BAPX_UI_URL}/static/favicon.png</Image>
    <Url type="text/html" method="get" template="{app.state.config.BAPX_UI_URL}/?q={"{searchTerms}"}"/>
    <moz:SearchForm>{app.state.config.BAPX_UI_URL}</moz:SearchForm>
