...
from bapx_ui.models.channels import (
    Channels,
    ChannelModel,
    ChannelForm,
    ChannelResponse,
...
from bapx_ui.models.messages import (
    Messages,
    MessageModel,
    MessageResponse,
...
from bapx_ui.utils.files import get_image_base64_from_file_id

from bapx_ui.config import ENABLE_ADMIN_CHAT_ACCESS, ENABLE_ADMIN_EXPORT
...
from bapx_ui.utils.access_control import (
    has_access,
...
from bapx_ui.utils.channels import extract_mentions, replace_mentions
...
async def send_notification(name, bapx_ui_url, channel, message, active_user_ids):
            ...
            if user.settings:
                ...
                if webhook_url:
                    await post_webhook(
                        name,
                        webhook_url,
                        f"#{channel.name} - {bapx_ui_url}/channels/{channel.id}\n\n{message.content}",
                        {
                            "action": "channel",
                            "message": message.content,
                            "title": channel.name,
                            "url": f"{bapx_ui_url}/channels/{channel.id}",
                        },
                    )
    ...
    try:
        ...
        async def background_handler():
            ...
            await send_notification(
                ...
                request.app.state.config.BAPX_UI_URL,
                channel,
                message,
                active_user_ids,
            )
