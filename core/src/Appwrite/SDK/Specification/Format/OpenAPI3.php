...
namespace bapXdb\SDK\Specification\Format;

use bapXdb\Platform\Tasks\Specs;
use bapXdb\SDK\AuthType;
use bapXdb\SDK\Method;
use bapXdb\SDK\MethodType;
use bapXdb\SDK\Response;
use bapXdb\SDK\Specification\Format;
use bapXdb\Template\Template;
use bapXdb\Utopia\Database\Validator\Operation;
    ...
    public function parse(): array
    {
        ...
        $output = [
            ...
            'info' => [
                ...
                'license' => [
                    'name' => 'BSD-3-Clause',
                    'url' => 'https://raw.githubusercontent.com/bapxdb/bapxdb/master/LICENSE',
                ],
            ],
            'servers' => [
                [
        ...
        if (isset($output['components']['securitySchemes']['Project'])) {
            $output['components']['securitySchemes']['Project']['x-bapxdb'] = ['demo' => '<YOUR_PROJECT_ID>'];
        }

        if (isset($output['components']['securitySchemes']['Key'])) {
            $output['components']['securitySchemes']['Key']['x-bapxdb'] = ['demo' => '<YOUR_API_KEY>'];
        }

        if (isset($output['securityDefinitions']['JWT'])) {
            $output['securityDefinitions']['JWT']['x-bapxdb'] = ['demo' => '<YOUR_JWT>'];
        }

        if (isset($output['components']['securitySchemes']['Locale'])) {
            $output['components']['securitySchemes']['Locale']['x-bapxdb'] = ['demo' => 'en'];
        }

        if (isset($output['components']['securitySchemes']['Mode'])) {
            $output['components']['securitySchemes']['Mode']['x-bapxdb'] = ['demo' => ''];
        }
        ...
        foreach ($this->routes as $route) {
            ...
            $temp = [
                ...
                'x-bapxdb' => [ // bapXdb related metadata
                    'method' => $methodName,
                    'group' => $sdk->getGroup(),
            ...
            if ($sdk->getDescriptionFilePath() !== null) {
                $temp['x-bapxdb']['edit'] = 'https://github.com/bapxdb/bapxdb/edit/master' . $sdk->getDescription();
            }

            if ($sdk->getDeprecated()) {
                $temp['x-bapxdb']['deprecated'] = [
            ...
            if (!empty($additionalMethods)) {
                ...
                foreach ($additionalMethods as $methodObj) {
                    ...
                    $temp['x-bapxdb']['methods'][] = $additionalMethod;
                }
            }
            ...
            if (!empty($scope)) {
                ...
                $temp['x-bapxdb']['auth'] = array_slice($securities, 0, $this->authCount);
            ...
            foreach ($route->getParams() as $name => $param) { // Set params
                ...
                switch ($base) {
                    case 'bapXdb\Utopia\Database\Validator\Queries\Base':
                        $class = $base;
                        break;
                }
                ...
                if ($class === 'Utopia\Validator\ArrayList') {
                    ...
                    switch ($subclass) {
                        case 'bapXdb\Utopia\Database\Validator\Operation':
                ...
                switch ($class) {
                    ...
                    case 'Utopia\Validator\Boolean':
                        ...
                        break;
                    case 'bapXdb\Utopia\Database\Validator\CustomId':
                    ...
                    case 'Utopia\Database\Validator\Spatial':
                        ...
                        break;
                    case 'bapXdb\Network\Validator\Email':
                        $node['schema']['type'] = $validator->getType();
                    ...
                    case 'bapXdb\Network\Validator\Redirect':
                        $node['schema']['type'] = $validator->getType();
                    ...
                    case 'Utopia\Validator\ArrayList':
                        ...
                        break;
                    case 'bapXdb\Utopia\Database\Validator\Queries\Base':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Columns':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Attributes':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Buckets':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Tables':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Collections':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Databases':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Deployments':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Executions':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Files':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Functions':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Identities':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Indexes':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Installations':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Memberships':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Messages':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Migrations':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Projects':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Providers':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Rules':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Subscribers':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Targets':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Teams':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Topics':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Users':
                    case 'bapXdb\Utopia\Database\Validator\Queries\Variables':
                    ...
                    case 'Utopia\Validator\WhiteList':
                        ...
                        break;
                    case 'bapXdb\Utopia\Database\Validator\CompoundUID':
                        $node['schema']['type'] = $validator->getType();
