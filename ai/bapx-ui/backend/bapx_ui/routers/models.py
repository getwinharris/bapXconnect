...
from bapx_ui.models.models import (
    ModelForm,
    ModelModel,
    ModelResponse,
    ModelListResponse,
...
from bapx_ui.utils.access_control import has_access, has_permission
from bapx_ui.config import BYPASS_ADMIN_ACCESS_CONTROL, STATIC_DIR
...
async def get_models(
    ...
    if not user.role == "admin" or not BYPASS_ADMIN_ACCESS_CONTROL:
        # For non-admin users, restrict results to the "bapX" model only
        # This ensures users only interact with the tool-aware sovereign model
        models_result = Models.search_models(user.id, filter=filter, skip=skip, limit=limit)
        models_result.items = [m for m in models_result.items if m.id == "bapX" or m.name == "bapX"]
        models_result.total = len(models_result.items)
