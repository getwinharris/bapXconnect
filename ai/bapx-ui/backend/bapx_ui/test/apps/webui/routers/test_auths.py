...
from test.util.mock_user import mock_bapx_ui_user
...
class TestAuths(AbstractPostgresTest):
    ...
    def test_get_session_user(self):
        with mock_bapx_ui_user():
            response = self.fast_api_client.get(self.create_url(""))
        assert response.status_code == 200
        assert response.json() == {
            "id": "1",
            "name": "John Doe",
            "email": "john.doe@openbapx_ui.com",
            "role": "user",
            "profile_image_url": "/user.png",
        }

    def test_update_profile(self):
        from bapx_ui.utils.auth import get_password_hash
        ...
        with mock_bapx_ui_user(id=user.id):
    ...
    def test_update_password(self):
        ...
        old_auth = self.auths.authenticate_user(
            "john.doe@openbapx_ui.com", "old_password"
        )
        assert old_auth is None
        new_auth = self.auths.authenticate_user(
            "john.doe@openbapx_ui.com", "new_password"
        )
    ...
    def test_signin(self):
        ...
        response = self.fast_api_client.post(
            ...
            json={"email": "john.doe@openbapx_ui.com", "password": "password"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["id"] == user.id
        assert data["name"] == "John Doe"
        assert data["email"] == "john.doe@openbapx_ui.com"
    ...
    def test_add_user(self):
        with mock_bapx_ui_user():
            response = self.fast_api_client.post(
                ...
                json={
                    "name": "John Doe 2",
                    "email": "john.doe2@openbapx_ui.com",
                    "password": "password2",
                    "role": "admin",
                },
            )
        ...
        assert data["email"] == "john.doe2@openbapx_ui.com"
