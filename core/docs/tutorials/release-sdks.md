...
This tutorial will cover how to properly release one or multiple bapXdb SDKs. The SDK release process involves updating the SDK generator, configuring Docker secrets, and running the release script.

### Prerequisites

Before releasing SDKs, you need to:

1. **Release a new SDK generator version** - Create a PR in the [sdk-generator](https://github.com/bapxdb/sdk-generator) repository with your respective sdk's changes. Wait for the PR to get merged and be released.
...
Add the SSH key volume mount to the `bapxdb` service in `docker-compose.yml`:
...
```bash
docker compose exec bapxdb specs
```

Also generate specs for the current stable bapXdb version:

```bash
docker compose exec bapxdb specs --version=1.8.x
```
...
```bash
docker compose exec bapxdb sdks --sdks=dart,flutter,cli,python
```
...
```text
Pull Request Summary
Dart: https://github.com/bapxdb/sdk-for-dart/pull/123
Flutter: https://github.com/bapxdb/sdk-for-flutter/pull/124
CLI: https://github.com/bapxdb/sdk-for-cli/pull/125
```

### Creating GitHub Releases

> **Note:** This section is for bapXdb maintainers only.

After the PRs have been reviewed and merged by an bapXdb Lead, you can create GitHub releases automatically.
...
```bash
docker compose exec bapxdb sdks --release=yes
```
...
[DRY RUN] Would create release for Dart SDK:
  Repository: bapxdb/sdk-for-dart
  Version: 13.0.0
  Title: 13.0.0
  Target Branch: main
...
```bash
docker compose exec bapxdb sdks --release=yes --commit=yes
```
...
- **`src/bapXdb/Platform/Tasks/SDKs.php`** - SDK generation and release logic
...
Congrats! You've successfully learned how to release bapXdb SDKs. Remember to:
