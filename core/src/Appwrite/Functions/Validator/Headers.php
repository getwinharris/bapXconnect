...
namespace bapXdb\Functions\Validator;
...
class Headers extends Validator
{
    ...
    {
        return 'Invalid headers: Alphanumeric characters or hyphens only, cannot start with "x-bapxdb", maximum ' . $this->maxKeys . ' keys, and total size ' . $this->maxSize . '.';
    }
    ...
    public function isValid($value): bool
    {
        ...
        foreach ($value as $key => $val) {
            ...
            // Check for x-bapxdb- prefix
            if (str_starts_with($key, 'x-bapxdb-')) {
                return false;
            }
        }
        return true;
