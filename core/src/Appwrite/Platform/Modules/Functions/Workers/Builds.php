...
namespace bapXdb\Platform\Modules\Functions\Workers;
...
class Builds extends Action
{
    ...
    protected function buildDeployment(
        ...
        if ($resource->getCollection() === 'functions' && empty($deployment->getAttribute('entrypoint', ''))) {
            throw new \Exception('Entrypoint for your bapXdb Function is missing. Please specify it when making deployment or update the entrypoint under your function\'s "Settings" > "Configuration" > "Entrypoint".');
        }
        ...
        try {
            ...
            if ($resource->getCollection() === 'sites') {
                ...
                $date = \date('H:i:s');
                $logs .= "[90m[$date] [90m[[0mbapxdb[90m][97m Screenshot capturing started. [0m\n";
                $deployment->setAttribute('buildLogs', $logs);
                $deployment = $dbForProject->updateDocument('deployments', $deployment->getId(), $deployment);
                $queueForRealtime
                    ->setPayload($deployment->getArrayCopy())
                ...
                try {
                    ...
                    $configs = [
                        'screenshotLight' => [
                            'headers' => [ 'x-bapxdb-hostname' => $rule->getAttribute('domain') ],
                            'url' => 'http://bapxdb/?bapxdb-preview=1&bapxdb-theme=light',
                            'theme' => 'light'
                        ],
                        'screenshotDark' => [
                            'headers' => [ 'x-bapxdb-hostname' => $rule->getAttribute('domain') ],
                            'url' => 'http://bapxdb/?bapxdb-preview=1&bapxdb-theme=dark',
                            'theme' => 'dark'
                        ],
                    ];
                        ...
                        return function () use ($key, $configs, $apiKey, $resource, $client, &$screenshotError) {
                            try {
                                ...
                                $config['headers'] = \array_merge($config['headers'] ?? [], [
                                    'x-bapxdb-key' => API_KEY_DYNAMIC . '_' . $apiKey
                                ]);
                                $config['sleep'] = 3000;
                                ...
                                $browserEndpoint = System::getEnv('_APP_BROWSER_HOST', 'http://bapxdb-browser:3000/v1');
                    ...
                    $logs = $deployment->getAttribute('buildLogs', '');
                    $date = \date('H:i:s');
                    $logs .= "[90m[$date] [90m[[0mbapxdb[90m][97m Screenshot capturing finished. [0m\n";
                    ...
                    $logs = $deployment->getAttribute('buildLogs', '');
                    $date = \date('H:i:s');
                    $logs .= "[90m[$date] [90m[[0mbapxdb[90m][33m Screenshot capturing failed. Deployment will continue. [0m\n";
            ...
            $logs = $deployment->getAttribute('buildLogs', '');
            $date = \date('H:i:s');
            $logs .= "[90m[$date] [90m[[0mbapxdb[90m][32m Deployment finished. [0m\n";
            $deployment->setAttribute('buildLogs', $logs);
    ...
    protected function runGitAction(
        ...
        try {
            ...
            $logs = $deployment->getAttribute('buildLogs', '');
            $date = \date('H:i:s');
            $logs .= "[90m[$date] [90m[[0mbapxdb[90m][33m Git action failed. Deployment will continue. [0m\n";
    ...
    private function cancelDeployment(string $deploymentId, Database $dbForProject, Realtime $queueForRealtime)
    {
        ...
        $logs = $deployment->getAttribute('buildLogs', '');
        $date = \date('H:i:s');
        $logs .= "[90m[$date] [90m[[0mbapxdb[90m][33m Build has been canceled. [0m\n";
