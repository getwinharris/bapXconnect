...
from bapx_ui.models.tags import TagModel, Tag, Tags
from bapx_ui.models.folders import Folders
from bapx_ui.utils.misc import sanitize_data_for_db, sanitize_text_for_db
...
class ChatTable:
    ...
    def search_chats_by_user_id(
        self, user_id: str, query: str, limit: int = 50
    ) -> list[ChatModel]:
        with get_db() as db:
            # Simple text search in title and chat content
            search_query = (
                select(Chat)
                ...
                .filter(
                    or_(
                        ...
                        text("CAST(chat AS TEXT) ILIKE :q").bindparams(q=f"%{query}%"),
                    )
                )
                .order_by(Chat.updated_at.desc())
                .limit(limit)
            )
            items = db.scalars(search_query).all()
            return [ChatModel.model_validate(item) for item in items]
