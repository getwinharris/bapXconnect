...
from bapx_ui.models.files import FileModel, FileUpdateForm, Files
from bapx_ui.models.knowledge import Knowledges
from bapx_ui.storage.provider import Storage


from bapx_ui.retrieval.vector.factory import VECTOR_DB_CLIENT

# Document loaders
from bapx_ui.retrieval.loaders.main import Loader
from bapx_ui.retrieval.loaders.youtube import YoutubeLoader

# bapX Sovereign Search Configuration
BAPX_SEARCH_URL = os.environ.get("BAPX_SEARCH_URL", "http://localhost/v1/bapx/search/loop")
BAPX_API_KEY = os.environ.get("BAPX_API_KEY", "sk-bapx-core")

# Web search engines
from bapx_ui.retrieval.web.main import SearchResult
from bapx_ui.retrieval.web.utils import get_web_loader
from bapx_ui.retrieval.web.ollama import search_ollama_cloud
from bapx_ui.retrieval.web.perplexity_search import search_perplexity_search
from bapx_ui.retrieval.web.brave import search_brave
from bapx_ui.retrieval.web.kagi import search_kagi
from bapx_ui.retrieval.web.mojeek import search_mojeek
from bapx_ui.retrieval.web.bocha import search_bocha
from bapx_ui.retrieval.web.duckduckgo import search_duckduckgo
from bapx_ui.retrieval.web.jina_search import search_jina
from bapx_ui.retrieval.web.searchapi import search_searchapi
from bapx_ui.retrieval.web.serpapi import search_serpapi
from bapx_ui.retrieval.web.searxng import search_searxng
from bapx_ui.retrieval.web.yacy import search_yacy
from bapx_ui.retrieval.web.serper import search_serper
from bapx_ui.retrieval.web.serply import search_serply
from bapx_ui.retrieval.web.serpstack import search_serpstack
from bapx_ui.retrieval.web.tavily import search_tavily
from bapx_ui.retrieval.web.bing import search_bing
from bapx_ui.retrieval.web.azure import search_azure
from bapx_ui.retrieval.web.exa import search_exa
from bapx_ui.retrieval.web.perplexity import search_perplexity
from bapx_ui.retrieval.web.sougou import search_sougou
from bapx_ui.retrieval.web.firecrawl import search_firecrawl
from bapx_ui.retrieval.web.external import search_external

from bapx_ui.retrieval.utils import (
...
from bapx_ui.retrieval.vector.utils import filter_metadata
from bapx_ui.utils.misc import (
    calculate_sha256_string,
    sanitize_text_for_db,
)
from bapx_ui.utils.auth import get_admin_user, get_verified_user
...
from bapx_ui.constants import ERROR_MESSAGES
...
def get_rf(
    ...
    if reranking_model:
        if any(model in reranking_model for model in ["jinaai/jina-colbert-v2"]):
            try:
                from bapx_ui.retrieval.models.colbert import ColBERT
            ...
            if engine == "external":
                try:
                    from bapx_ui.retrieval.models.external import ExternalReranker
...
async def process_web_search(
    ...
    # bapX Sovereign Context Retrieval (Cross-Session)
    bapx_docs = []
    try:
        # Directional Step 1: Query bapX Sovereign Memory (Cross-Session)
        for query in form_data.queries:
            # We use a Grok-like dynamic search loop to get deep context
            params = {"query": query, "circles": 3}
            headers = {"Authorization": f"Bearer {BAPX_API_KEY}"}
            response = requests.get(BAPX_SEARCH_URL, params=params, headers=headers, timeout=10)
                ...
                if data.get("context"):
                    bapx_docs.append(
                        Document(
                            ...
                            metadata={
                                "source": "bapX Sovereign Memory",
                                "title": "Cross-Session Context Retrieval",
                                "cross_session": True
                            }
                        )
                    )
    except Exception as e:
        log.warning(f"bapX Cross-Session Search failed: {e}")
    ...
    try:
        ...
        # Directional Step 2: Merge Sovereign Context into Web Results
        if bapx_docs:
            docs = bapx_docs + docs
