...
struct State {
  ...
  pg_conn: Arc<tokio::sync::Mutex<Option<bb8::PooledConnection<'static, bb8_postgres::PostgresConnectionManager<tokio_postgres::NoTls>>>>>,
    pg_tx: Arc<tokio::sync::Mutex<Option<postgres::OwnedPgTx>>>,
}
...
impl WasiHttpView for State {
  ...
  fn send_request(
    ...
    return match request.uri().host() {
      ...
      Some("__postgres") => {
        let pool = self.shared.pg_pool.clone();
        let pg_conn = self.pg_conn.clone();
        let pg_tx = self.pg_tx.clone();
        Ok(
          wasmtime_wasi_http::types::HostFutureIncomingResponse::pending(
            wasmtime_wasi::runtime::spawn(async move {
              let mut pg_conn_lock = pg_conn.lock().await;
              let mut pg_tx_lock = pg_tx.lock().await;

              if pg_conn_lock.is_none() {
                let Some(pool) = pool else {
                  return Ok(Err(ErrorCode::InternalError(Some("PostgreSQL not configured".to_string()))));
                };
                let client = pool.get().await.map_err(|e| ErrorCode::InternalError(Some(e.to_string())))?;
                *pg_conn_lock = Some(client);
              }

              let client = pg_conn_lock.as_mut().expect("just set");

              // Handle transactions
               match request.uri().path() {
                 "/tx-begin" => {
                   if pg_tx_lock.is_some() {
                     return Ok(postgres::handle_postgres_request(client, request).await); // Let it fail in postgres.rs
                   }
                   let resp = postgres::handle_postgres_request(client, request).await;
                   if resp.is_ok() {
                     // Since we don't actually store a Transaction object yet, we just track that we called BEGIN
                     // In a real implementation, we'd want to store something that represents the open transaction.
                   }
                   Ok(resp)
                 }
                 ...
                 _ => {
                   Ok(postgres::handle_postgres_request(client, request).await)
                 }
               }
            }),
          ),
...
impl Runtime {
  pub fn new(
    ...
    conn: trailbase_sqlite::Connection,
    pg_pool: Option<bb8::Pool<bb8_postgres::PostgresConnectionManager<tokio_postgres::NoTls>>>,
    kv_store: KvStore,
    opts: RuntimeOptions,
  ) -> Result<Self, Error> {
    let engine = {
...
pub struct SharedState {
  ...
  pub pg_pool: Option<bb8::Pool<bb8_postgres::PostgresConnectionManager<tokio_postgres::NoTls>>>,
  ...
  fn new_store(&self) -> Result<Store<State>, Error> {
    ...
    return Ok(Store::new(
      ...
      State {
        ...
        pg_conn: Arc::new(tokio::sync::Mutex::new(None)),
        pg_tx: Arc::new(tokio::sync::Mutex::new(None)),
      },
    ));
  }
