...
* **Feat:** Add support for `bapxdb.config.json` file
...
> **Note:** The existing `bapxdb.json` file remains fully supported for backward compatibility
...
> **Note:** Storing environment variables in the `vars` attribute of `bapxdb.json` is now deprecated due to security risks. Variables are now synced directly from the `.env` file in the root directory of the function’s or site’s folder.
...
* Type generation fixes and improvements:
  * Add `--strict` / `-s` flag to `bapxdb types` command to generate types in strict mode. This automatically converts the casing of attributes to match the language's naming conventions
...
* Type generation fixes:
  * Add ability to generate types directly to a specific file by passing a file path to `bapxdb types output_path`, instead of just a directory
...
* Update `init` command to clear previous configuration in `bapxdb.json`
