"""OpenTelemetry metrics bootstrap for bapX UI.
...
def _build_meter_provider(resource: Resource) -> MeterProvider:
    ...
    views: List[View] = [
        ...
        View(
            instrument_name="bapx_ui.users.total",
        ),
        View(
            instrument_name="bapx_ui.users.active",
        ),
        View(
            instrument_name="bapx_ui.users.active.today",
        ),
    ]
...
def setup_metrics(app: FastAPI, resource: Resource) -> None:
    ...
    meter.create_observable_gauge(
        name="bapx_ui.users.active.today",
