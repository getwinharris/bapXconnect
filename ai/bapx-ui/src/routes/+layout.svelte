<script>
	...
	import { BAPX_UI_API_BASE_URL, BAPX_UI_BASE_URL, BAPX_UI_HOSTNAME } from '$lib/constants';
	...
	const setupSocket = async (enableWebsocket) => {
		const _socket = io(`${BAPX_UI_BASE_URL}` || undefined, {
			reconnection: true,
		...
		_socket.on('connect', async () => {
			...
			if (version !== null || deploymentId !== null) {
				if (
					($BAPX_UI_VERSION !== null && version !== $BAPX_UI_VERSION) ||
					($BAPX_UI_DEPLOYMENT_ID !== null && deploymentId !== $BAPX_UI_DEPLOYMENT_ID)
				) {
					await unregisterServiceWorkers();
			...
			if (deploymentId !== null) {
				BAPX_UI_DEPLOYMENT_ID.set(deploymentId);
			}

			if (version !== null) {
				BAPX_UI_VERSION.set(version);
			}
				...
				if (done) {
					...
					if ($isLastActiveTab) {
						if ($settings?.notificationEnabled ?? false) {
							new Notification(`${title} • bapX AI`, {
								body: content,
								icon: `/favicon.png`
							});
						}
			...
			if (type === 'message') {
				...
				if ($isLastActiveTab) {
					if ($settings?.notificationEnabled ?? false) {
						new Notification(`${title} • bapX AI`, {
							body: data?.content,
							icon: `${BAPX_UI_API_BASE_URL}/users/${data?.user?.id}/profile/image`
						});
					}
				}
	...
	onMount(async () => {
		...
		if (backendConfig) {
				...
				await BAPX_UI_NAME.set('bapX AI');
...
<svelte:head>
	<title>{$BAPX_UI_NAME}</title>
	<link crossorigin="anonymous" rel="icon" href="{BAPX_UI_BASE_URL}/static/favicon.png" />

	<meta name="apple-mobile-web-app-title" content={$BAPX_UI_NAME} />
	<meta name="description" content={$BAPX_UI_NAME} />
	<link
		rel="search"
