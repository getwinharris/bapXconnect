...
from bapx_ui.models.auths import (
    AddUserForm,
    ApiKey,
    Auths,
    Token,
...
from bapx_ui.models.users import (
    UserProfileImageResponse,
    Users,
    UpdateProfileForm,
    UserStatus,
)
from bapx_ui.models.groups import Groups
from bapx_ui.models.oauth_sessions import OAuthSessions

from bapx_ui.constants import ERROR_MESSAGES, WEBHOOK_MESSAGES
from bapx_ui.env import (
    BAPX_UI_AUTH,
    BAPX_UI_AUTH_TRUSTED_EMAIL_HEADER,
    BAPX_UI_AUTH_TRUSTED_NAME_HEADER,
    BAPX_UI_AUTH_TRUSTED_GROUPS_HEADER,
    BAPX_UI_AUTH_COOKIE_SAME_SITE,
...
from bapx_ui.utils.misc import parse_duration, validate_email_format
...
from bapx_ui.utils.webhook import post_webhook
from bapx_ui.utils.access_control import get_permissions, has_permission
from bapx_ui.utils.groups import apply_default_group_assignment

from bapx_ui.utils.redis import get_redis_client
from bapx_ui.utils.rate_limit import RateLimiter
...
async def get_session_user(
    ...
    if data:
        ...
        response.set_cookie(
            ...
            samesite=BAPX_UI_AUTH_COOKIE_SAME_SITE,
            secure=BAPX_UI_AUTH_COOKIE_SECURE,
        )
...
async def update_password(
    ...
    if BAPX_UI_AUTH_TRUSTED_EMAIL_HEADER:
...
async def signin(request: Request, response: Response, form_data: SigninForm):
    ...
    if BAPX_UI_AUTH_TRUSTED_EMAIL_HEADER:
        if BAPX_UI_AUTH_TRUSTED_EMAIL_HEADER not in request.headers:
            raise HTTPException(400, detail=ERROR_MESSAGES.INVALID_TRUSTED_HEADER)

        email = request.headers[BAPX_UI_AUTH_TRUSTED_EMAIL_HEADER].lower()
        name = email

        if BAPX_UI_AUTH_TRUSTED_NAME_HEADER:
            name = request.headers.get(BAPX_UI_AUTH_TRUSTED_NAME_HEADER, email)
            try:
        ...
        user = Auths.authenticate_user_by_email(email)
        if BAPX_UI_AUTH_TRUSTED_GROUPS_HEADER and user and user.role != "admin":
    ...
    try:
        ...
        if user:
            ...
            if request.app.state.config.WEBHOOK_URL:
                await post_webhook(
                    request.app.state.BAPX_UI_NAME,
...
async def signout(request: Request, response: Response):
                            ...
                            if logout_url:
                                return JSONResponse(
                                    ...
                                    content={
                                        ...
                                        + (
                                            f"&post_logout_redirect_uri={BAPX_UI_AUTH_SIGNOUT_REDIRECT_URL}"
                                            if BAPX_UI_AUTH_SIGNOUT_REDIRECT_URL
                                            else ""
                                        ),
                                    },
    ...
    if BAPX_UI_AUTH_SIGNOUT_REDIRECT_URL:
        return JSONResponse(
            ...
            content={
                "status": True,
                "redirect_url": BAPX_UI_AUTH_SIGNOUT_REDIRECT_URL,
            },
            headers=response.headers,
        )
...
async def get_admin_config(request: Request, user=Depends(get_admin_user)):
    return {
        ...
        "BAPX_UI_URL": request.app.state.config.BAPX_UI_URL,
...
async def update_admin_config(
    ...
    request.app.state.config.BAPX_UI_URL = form_data.BAPX_UI_URL
