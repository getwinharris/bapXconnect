...
use Ahc\Jwt\JWT;
use bapXdb\Auth\Validator\Phone;
use bapXdb\Detector\Detector;
use bapXdb\Event\Delete;
use bapXdb\Event\Event;
use bapXdb\Event\Messaging;
use bapXdb\Extend\Exception;
use bapXdb\Messaging\Status as MessageStatus;
use bapXdb\Network\Validator\Email;
...
use bapXdb\Utopia\Database\Validator\CompoundUID;
use bapXdb\Utopia\Database\Validator\CustomId;
use bapXdb\Utopia\Database\Validator\Queries\Messages;
use bapXdb\Utopia\Database\Validator\Queries\Providers;
use bapXdb\Utopia\Database\Validator\Queries\Subscribers;
use bapXdb\Utopia\Database\Validator\Queries\Targets;
use bapXdb\Utopia\Database\Validator\Queries\Topics;
...
App::get('/v1/messaging/providers')
    ...
    ->param('queries', [], new Providers(), 'Array of query strings generated using the Query class provided by the SDK. [Learn more about queries](https://bapxdb.io/docs/queries). Maximum of ' . APP_LIMIT_ARRAY_PARAMS_SIZE . ' queries are allowed, each ' . APP_LIMIT_ARRAY_ELEMENT_SIZE . ' characters long. You may filter on the following attributes: ' . implode(', ', Providers::ALLOWED_ATTRIBUTES), true)
...
App::get('/v1/messaging/providers/:providerId/logs')
    ...
    ->param('queries', [], new Queries([new Limit(), new Offset()]), 'Array of query strings generated using the Query class provided by the SDK. [Learn more about queries](https://bapxdb.io/docs/queries). Only supported methods are limit and offset', true)
    ->param('total', true, new Boolean(true), 'When set to false, the total count returned will be 0 and will not be calculated.', true)
    ->inject('response')
    ->inject('dbForProject')
    ->inject('locale')
...
App::get('/v1/messaging/topics')
    ...
    ->param('queries', [], new Topics(), 'Array of query strings generated using the Query class provided by the SDK. [Learn more about queries](https://bapxdb.io/docs/queries). Maximum of ' . APP_LIMIT_ARRAY_PARAMS_SIZE . ' queries are allowed, each ' . APP_LIMIT_ARRAY_ELEMENT_SIZE . ' characters long. You may filter on the following attributes: ' . implode(', ', Topics::ALLOWED_ATTRIBUTES), true)
    ->param('search', '', new Text(256), 'Search term to filter your list results. Max length: 256 chars.', true)
    ->param('total', true, new Boolean(true), 'When set to false, the total count returned will be 0 and will not be calculated.', true)
    ->inject('dbForProject')
    ->inject('authorization')
...
App::post('/v1/messaging/messages/push')
    ...
    ->param('image', '', new CompoundUID(), 'Image for push notification. Must be a compound bucket ID to file ID of a jpeg, png, or bmp image in bapXdb Storage. It should be formatted as <BUCKET_ID>:<FILE_ID>.', true)
...
App::get('/v1/messaging/messages')
    ...
    ->param('queries', [], new Messages(), 'Array of query strings generated using the Query class provided by the SDK. [Learn more about queries](https://bapxdb.io/docs/queries). Maximum of ' . APP_LIMIT_ARRAY_PARAMS_SIZE . ' queries are allowed, each ' . APP_LIMIT_ARRAY_ELEMENT_SIZE . ' characters long. You may filter on the following attributes: ' . implode(', ', Messages::ALLOWED_ATTRIBUTES), true)
    ->param('search', '', new Text(256), 'Search term to filter your list results. Max length: 256 chars.', true)
    ->param('total', true, new Boolean(true), 'When set to false, the total count returned will be 0 and will not be calculated.', true)
    ->inject('dbForProject')
    ->inject('authorization')
...
App::get('/v1/messaging/messages/:messageId/targets')
    ...
    ->param('messageId', '', new UID(), 'Message ID.')
    ->param('queries', [], new Targets(), 'Array of query strings generated using the Query class provided by the SDK. [Learn more about queries](https://bapxdb.io/docs/queries). Maximum of ' . APP_LIMIT_ARRAY_PARAMS_SIZE . ' queries are allowed, each ' . APP_LIMIT_ARRAY_ELEMENT_SIZE . ' characters long. You may filter on the following attributes: ' . implode(', ', Targets::ALLOWED_ATTRIBUTES), true)
    ->param('total', true, new Boolean(true), 'When set to false, the total count returned will be 0 and will not be calculated.', true)
    ->inject('response')
    ->inject('dbForProject')
...
App::patch('/v1/messaging/messages/push/:messageId')
    ...
    ->param('image', null, new Nullable(new CompoundUID()), 'Image for push notification. Must be a compound bucket ID to file ID of a jpeg, png, or bmp image in bapXdb Storage. It should be formatted as <BUCKET_ID>:<FILE_ID>.', true)
