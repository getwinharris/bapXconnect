...
use Utopia\Migration\Destinations\bapXdb as DestinationbapXdb;
use Utopia\Migration\Destinations\CSV as DestinationCSV;
use Utopia\Migration\Exception as MigrationException;
use Utopia\Migration\Source;
use Utopia\Migration\Sources\bapXdb;
use Utopia\Migration\Sources\bapXdb as SourcebapXdb;
...
class Migrations extends Action
{
    ...
    protected function processSource(Document $migration, array $platform): Source
    {
        ...
        $dataSource = bapXdb::SOURCE_API;
        $database = null;
        $queries = [];

        if ($source === bapXdb::getName() && $destination === DestinationCSV::getName()) {
            $dataSource = bapXdb::SOURCE_DATABASE;
        ...
        $migrationSource = match ($source) {
            ...
            SourcebapXdb::getName() => new SourcebapXdb(
    ...
    protected function processDestination(Document $migration, string $apiKey, array $platform): Destination
    {
        ...
        return match ($destination) {
            DestinationbapXdb::getName() => new DestinationbapXdb(
                $this->project->getId(),
    ...
    protected function processMigration(
        ...
        try {
            if (
                $migration->getAttribute('source') === SourcebapXdb::getName() &&
            ...
            if (! $migration->isEmpty()) {
                ...
                call_user_func($this->logError, $th, 'bapxdb-worker', 'bapxdb-queue-'.self::getName(), [
            ...
            if ($migration->getAttribute('status', '') === 'failed') {
                ...
                foreach ([...$sourceErrors, ...$destinationErrors] as $error) {
                    ...
                    if ($error->getCode() === 0 || $error->getCode() >= 500) {
                        ($this->logError)($error, 'bapxdb-worker', 'bapxdb-queue-' . self::getName(), [
                            'migrationId' => $migration->getId(),
                            'source' => $migration->getAttribute('source') ?? '',
                            'destination' => $migration->getAttribute('destination') ?? '',
