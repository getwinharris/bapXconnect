...
use bapXdb\Auth\Validator\Password;
use bapXdb\Auth\Validator\PasswordDictionary;
use bapXdb\Auth\Validator\PasswordHistory;
use bapXdb\Auth\Validator\PersonalData;
use bapXdb\Auth\Validator\Phone;
use bapXdb\Deletes\Identities as DeleteIdentities;
use bapXdb\Deletes\Targets as DeleteTargets;
use bapXdb\Detector\Detector;
use bapXdb\Event\Delete;
use bapXdb\Event\Event;
use bapXdb\Extend\Exception;
use bapXdb\Hooks\Hooks;
...
App::get('/v1/users')
    ...
    ->param('queries', [], new Users(), 'Array of query strings generated using the Query class provided by the SDK. [Learn more about queries](https://bapxdb.io/docs/queries). Maximum of ' . APP_LIMIT_ARRAY_PARAMS_SIZE . ' queries are allowed, each ' . APP_LIMIT_ARRAY_ELEMENT_SIZE . ' characters long. You may filter on the following attributes: ' . implode(', ', Users::ALLOWED_ATTRIBUTES), true)
...
App::get('/v1/users/identities')
    ...
    ->param('queries', [], new Identities(), 'Array of query strings generated using the Query class provided by the SDK. [Learn more about queries](https://bapxdb.io/docs/queries). Maximum of ' . APP_LIMIT_ARRAY_PARAMS_SIZE . ' queries are allowed, each ' . APP_LIMIT_ARRAY_ELEMENT_SIZE . ' characters long. You may filter on the following attributes: ' . implode(', ', Identities::ALLOWED_ATTRIBUTES), true)
    ->param('search', '', new Text(256), 'Search term to filter your list results. Max length: 256 chars.', true)
    ->param('total', true, new Boolean(true), 'When set to false, the total count returned will be 0 and will not be calculated.', true)
    ->inject('response')
    ->inject('dbForProject')
